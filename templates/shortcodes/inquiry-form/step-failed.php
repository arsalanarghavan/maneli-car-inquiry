        ...
        <div class="card custom-card">
            <div class="card-header">
                <div class="card-title">
                    ...
                    <?php esc_html_e('Inquiry Unsuccessful', 'autopuzzle'); ?>
                </div>
            </div>
            <div class="card-body">
                <div class="alert alert-danger" role="alert">
                    <i class="la la-times-circle me-2"></i>
                    <?php esc_html_e('Unfortunately, an error occurred while attempting to retrieve your inquiry from the banking system. This issue could be due to incorrect identity information or temporary system problems.', 'autopuzzle'); ?>
                </div>

                <form class="mt-4" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post">
                    <input type="hidden" name="action" value="autopuzzle_retry_inquiry">
                    <input type="hidden" name="inquiry_id" value="<?php echo esc_attr($inquiry_id); ?>">
                    <?php wp_nonce_field('autopuzzle_retry_inquiry_nonce'); ?>
                    
                    <p class="text-muted mb-3"><?php esc_html_e('You can edit your information and try again.', 'autopuzzle'); ?></p>
                    
                    <button type="submit" class="btn btn-primary btn-wave">
                        <i class="la la-sync me-1"></i>
                        <?php esc_html_e('Retry and Correct Information', 'autopuzzle'); ?>
                    </button>
                </form>
            </div>
        </div>
