...
$phone = '';
if (isset($_SESSION) && isset($_SESSION['autopuzzle_sms_phone'])) {
    $phone = sanitize_text_field($_SESSION['autopuzzle_sms_phone']);
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?php echo esc_html( sprintf( __('Verify Code - %s', 'autopuzzle'), autopuzzle_brand_name() ) ); ?></title>
    ...
    <script src="<?php echo AUTOPUZZLE_PLUGIN_URL; ?>assets/js/two-step-verification.js"></script>
</head>
                    ...
                    <div class="card custom-card my-auto border">
                        <div class="card-body p-4 p-sm-5">
                            ...
                            <p class="mb-4 text-muted op-7 fw-normal text-center fs-12">
                                ...
                                echo sprintf(esc_html__('Enter the 4-digit code sent to mobile number %s.', 'autopuzzle'), $masked_phone); 
                                ?>
                            </p>
                            <div class="row gy-3">
                                <div class="col-xl-12 mb-2">
                                    ...
                                    <div class="form-check mt-2">
                                        ...
                                        <label class="form-check-label fs-14" for="resend-check">
                                            <?php echo esc_html(__("Didn't receive code?", 'autopuzzle')); ?>
                                            <a href="javascript:void(0);" class="text-primary ms-2 d-inline-block" id="resend-link"><?php echo esc_html(__('Resend', 'autopuzzle')); ?></a>
                                        </label>
                                    </div>
                                </div>
                                <div class="col-xl-12 d-grid mt-2">
                                    <button id="verify-btn" class="btn btn-primary" onclick="verifyOtpCode()"><?php echo esc_html(__('Verify', 'autopuzzle')); ?></button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="authentication-cover overflow-hidden">
                ...
                <div class="aunthentication-cover-content d-flex align-items-center justify-content-center">
                    <div>
                        ...
                        <h6 class="text-fixed-white mb-3 fw-medium"><?php echo esc_html(__('Two-step Verification', 'autopuzzle')); ?></h6>
                        <p class="text-fixed-white mb-1 op-6"><?php echo esc_html(__('Please enter the verification code sent to your mobile number to continue.', 'autopuzzle')); ?></p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        var autopuzzle_ajax = {
            ...
            strings: {
                ...
                verify: '<?php echo esc_js(__('Verify', 'autopuzzle')); ?>',
                invalidCode: '<?php echo esc_js(__('Please enter the complete 4-digit code.', 'autopuzzle')); ?>',
                resending: '<?php echo esc_js(__('Sending...', 'autopuzzle')); ?>',
                resend: '<?php echo esc_js(__('Resend', 'autopuzzle')); ?>'
            }
        };
        ...
        function verifyOtpCode() {
            ...
            if (code.length !== 4) {
                alert(autopuzzle_ajax.strings.invalidCode);
                return;
            }
            ...
            .then(data => {
                if (data.success) {
                    ...
                    btn.disabled = false;
                    btn.innerHTML = autopuzzle_ajax.strings.verify;
                    // Clear inputs
        ...
        document.getElementById('resend-link').addEventListener('click', function(e) {
            ...
            link.textContent = autopuzzle_ajax.strings.resending;
            ...
            .then(data => {
                ...
                link.textContent = autopuzzle_ajax.strings.resend;
            })
            .catch(error => {
                console.error('Error:', error);
                alert('<?php echo esc_js(__('An error occurred. Please try again.', 'autopuzzle')); ?>');
