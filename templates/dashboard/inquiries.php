...
// Get experts for filter dropdown
                        ...
                        <tbody>
                            ...
                            if ($show_type === 'cash' || $show_type === 'all') {
                                ...
                                if (!empty($_GET['search'])) {
                                    ...
                                    $product_ids = wc_get_products(['s' => $search_term, 'limit' => -1, 'return' => 'ids']);
                                    $search_meta_query = [
                                        'relation' => 'OR', 
                                        ['key' => 'cash_first_name', 'value' => $search_term, 'compare' => 'LIKE'], 
                                        ['key' => 'cash_last_name', 'value' => $search_term, 'compare' => 'LIKE'], 
                                ...
                                if (!empty($_GET['status'])) {
                                    ...
                                    if ($status_filter === 'new') {
                                        $meta_query[] = [
                                            ...
                                            ['key' => 'cash_inquiry_status', 'value' => null, 'compare' => '=']
                                        ];
                                    } else {
                                        $meta_query[] = ['key' => 'cash_inquiry_status', 'value' => $status_filter, 'compare' => '='];
                                    }
                                }
                                
                                // Expert filter
                                if (!empty($_GET['expert'])) {
                                    $meta_query[] = ['key' => 'assigned_expert_id', 'value' => absint($_GET['expert']), 'compare' => '='];
                                }
                                
                                // SECURITY: Filter by user role
                                if ($is_expert && !$is_admin) {
                                    // Expert sees only assigned inquiries
                                    $meta_query[] = ['key' => 'assigned_expert_id', 'value' => $current_user_id, 'compare' => '='];
                                } elseif ($is_customer) {
                                    // Customer sees only their own inquiries
                                ...
                                if (count($meta_query) > 1) {
                                    $cash_args['meta_query'] = $meta_query;
                                }
                                
                                
                                // Apply sorting
                                $sort = !empty($_GET['sort']) ? sanitize_text_field($_GET['sort']) : 'default';
                                if ($sort === 'default') {
                                    // Default: oldest inquiries first
                                    $cash_args['orderby'] = 'date';
                                    $cash_args['order'] = 'ASC';
                                } elseif ($sort === 'date_desc') {
                                    $cash_args['orderby'] = 'date';
                                    $cash_args['order'] = 'DESC';
                            ...
                            if (!empty($all_inquiries)) {
                                ...
                                if ($sort === 'default') {
                                    // Default: oldest inquiries first (when status filter is applied, just sort by date)
                                    if (!empty($_GET['status'])) {
                                        // If status filter is applied, just sort by date
                                        usort($all_inquiries, function($a, $b) {
                                            return $a['timestamp'] - $b['timestamp']; // Older first
                                        });
                                    } else {
                                        // If no status filter, prioritize new inquiries
                                        usort($all_inquiries, function($a, $b) {
                                        // First sort by status priority (new first, then referred, then in_progress)
                                        $status_priority = [
                                            'new' => 1, 
                                            'referred' => 2, 
                                            'in_progress' => 3, 
                                            'follow_up_scheduled' => 4,
                                            'awaiting_downpayment' => 5,
                                            ...
                                            $a_priority = isset($status_priority[$a['status']]) ? $status_priority[$a['status']] : 11;
                                            $b_priority = isset($status_priority[$b['status']]) ? $status_priority[$b['status']] : 11;
...
if ($count_type === 'cash') {
    // Enqueue inquiry lists assets for filtering
    maneli_enqueue_inquiry_report_assets();
    ...
    add_action('wp_footer', function() {
        ...
        <!-- Cash Inquiry Filters JavaScript -->
        <script>
        jQuery(document).ready(function($) {
            // Function to apply filters
            function applyFilters() {
                var search = $('#cash-inquiry-search-input').val();
                var status = $('#cash-inquiry-status-filter').val();
                var expert = $('#cash-expert-filter').val();
                var sort = $('#cash-inquiry-sort-filter').val();
                
                // Hide table and show loading
                $('#datatable-basic tbody').html('<tr><td colspan="8" class="text-center py-4"><i class="la la-spinner la-spin fs-24 text-muted"></i><p class="mt-2 text-muted">در حال بارگذاری...</p></td></tr>');
                
                // Build query parameters
                var params = new URLSearchParams();
                
                // Always add parameters, even if empty (to clear them)
                if (search) {
            ...
            $('#cash-inquiry-apply-filters').on('click', function(e) {
                e.preventDefault();
                applyFilters();
            });
            
            // Reset filters button
            $('#cash-inquiry-reset-filters').on('click', function(e) {
                e.preventDefault();
                $('#cash-inquiry-search-input').val('');
                $('#cash-inquiry-status-filter').val('');
                $('#cash-expert-filter').val('');
            ...
            $('#cash-inquiry-status-filter, #cash-expert-filter, #cash-inquiry-sort-filter').on('change', function() {
                applyFilters();
            });
            
            // Search with debounce
            var searchTimeout;
            $('#cash-inquiry-search-input').on('input', function() {
            ...
            if (urlParams.get('search')) $('#cash-inquiry-search-input').val(urlParams.get('search'));
            if (urlParams.get('status')) $('#cash-inquiry-status-filter').val(urlParams.get('status'));
            if (urlParams.get('expert')) $('#cash-expert-filter').val(urlParams.get('expert'));
            if (urlParams.get('sort')) $('#cash-inquiry-sort-filter').val(urlParams.get('sort'));
        });
        </script>
