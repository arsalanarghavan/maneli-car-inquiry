            ...
            <div>
                <nav>
                    <ol class="breadcrumb mb-1">
                        ...
                        <li class="breadcrumb-item active" aria-current="page"><?php esc_html_e('Telegram Notifications', 'autopuzzle'); ?></li>
                    </ol>
                </nav>
                <h1 class="page-title mb-0"><?php esc_html_e('Telegram Notifications', 'autopuzzle'); ?></h1>
            </div>
        </div>
        <!-- End::page-header -->
            ...
            <div class="col-xl col-lg-4 col-md-6 col-sm-6 col-6">
                <div class="card custom-card crm-card overflow-hidden">
                    <div class="card-body">
                        ...
                        <p class="flex-fill text-muted fs-14 mb-1"><?php esc_html_e('Total Telegram', 'autopuzzle'); ?></p>
                        <div class="d-flex align-items-center justify-content-between mt-1">
                ...
                <div class="card custom-card crm-card overflow-hidden">
                    <div class="card-body">
                        ...
                        <div class="d-flex align-items-center justify-content-between mt-1">
                            ...
                            <span class="badge bg-info-transparent rounded-pill fs-11"><?php esc_html_e('Messages', 'autopuzzle'); ?></span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card custom-card crm-card overflow-hidden">
                    <div class="card-body">
                        ...
                        <div class="d-flex align-items-center justify-content-between mt-1">
                            ...
                            <span class="badge bg-secondary-transparent rounded-pill fs-11"><?php esc_html_e('Messages', 'autopuzzle'); ?></span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row mb-4">
            <div class="col-xl-6">
                <div class="card custom-card">
                    <div class="card-header">
                        <h5 class="mb-0"><?php esc_html_e('Telegram Timeline', 'autopuzzle'); ?></h5>
                    </div>
                    <div class="card-body">
        ...
        <div class="card custom-card mb-4">
            <div class="card-header">
                <div class="d-flex align-items-center justify-content-between">
                    <h5 class="mb-0"><?php esc_html_e('Telegram Logs', 'autopuzzle'); ?></h5>
                            ...
                            <?php if (empty($logs)): ?>
                                <tr>
                                    <td colspan="7" class="text-center py-4">
                                        <p class="text-muted mb-0"><?php esc_html_e('No Telegram notifications found.', 'autopuzzle'); ?></p>
                                    </td>
                                </tr>
                            <?php else: ?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?php esc_html_e('Send Single Telegram', 'autopuzzle'); ?></h5>
            ...
            <div class="modal-body">
                <form id="sendSingleForm">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="recipient" id="singleRecipient" class="form-control" required placeholder="<?php esc_attr_e('Chat ID or User ID', 'autopuzzle'); ?>">
                        <small class="form-text text-muted"><?php esc_html_e('Enter Telegram Chat ID or user ID', 'autopuzzle'); ?></small>
                    </div>
                    <div class="mb-3">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-success" id="sendSingleBtn">
                    <i class="ri-send-plane-line me-1"></i>
                    <?php esc_html_e('Send Telegram', 'autopuzzle'); ?>
                </button>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="sendBulkModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?php esc_html_e('Send Bulk Telegram', 'autopuzzle'); ?></h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <form id="bulkNotificationForm">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="custom_recipients" id="bulkCustomRecipients" class="form-control" rows="3" placeholder="<?php esc_attr_e('Or enter custom Chat IDs (one per line)', 'autopuzzle'); ?>"></textarea>
...
<div class="modal fade" id="scheduleModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?php esc_html_e('Schedule Telegram', 'autopuzzle'); ?></h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <form id="scheduleNotificationForm">
                    ...
                    <div class="mb-3">
                        <label class="form-label"><?php esc_html_e('Recipient', 'autopuzzle'); ?> <span class="text-danger">*</span></label>
                        <input type="text" name="recipient" id="scheduleRecipient" class="form-control" required placeholder="<?php esc_attr_e('Chat ID or User ID', 'autopuzzle'); ?>">
                    </div>
                    <div class="mb-3">
                        <label class="form-label"><?php esc_html_e('Message', 'autopuzzle'); ?> <span class="text-danger">*</span></label>
...
jQuery(document).ready(function($) {
                    ...
                    if (response.success && response.data) {
                        var timelineChart = new Chart(timelineCtx, {
                            ...
                            data: {
                                ...
                                datasets: [{
                                    label: '<?php echo esc_js(__('Sent Telegram', 'autopuzzle')); ?>',
    ...
    $('#sendSingleBtn').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    alert('<?php echo esc_js(__('Telegram sent successfully!', 'autopuzzle')); ?>');
            ...
            complete: function() {
                btn.prop('disabled', false).html('<i class="ri-send-plane-line me-1"></i> <?php echo esc_js(__('Send Telegram', 'autopuzzle')); ?>');
            }
        });
    });
    ...
    $('#sendBulkBtn').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    alert('<?php echo esc_js(__('Bulk Telegram sent successfully!', 'autopuzzle')); ?>');
    ...
    $('#scheduleBtn').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    alert('<?php echo esc_js(__('Telegram scheduled successfully!', 'autopuzzle')); ?>');
    ...
    $('.retry-notification').on('click', function() {
        ...
        if (!confirm('<?php echo esc_js(__('Retry sending this Telegram?', 'autopuzzle')); ?>')) {
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    alert('<?php echo esc_js(__('Telegram retried successfully!', 'autopuzzle')); ?>');
                    location.reload();
                } else {
                    alert('<?php echo esc_js(__('Error:', 'autopuzzle')); ?> ' + (response.data.message || 'Unknown error'));
                }
