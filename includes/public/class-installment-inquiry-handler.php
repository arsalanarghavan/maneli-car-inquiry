...
class Autopuzzle_Installment_Inquiry_Handler {
    ...
    public function __construct() {
        ...
        add_action('autopuzzle_cash_inquiry_payment_successful', [$this, 'finalize_cash_inquiry_from_hook'], 10, 2);
    ...
    public function ajax_get_meeting_slots() {
        check_ajax_referer('autopuzzle_meetings_nonce', 'nonce');
        ...
        if ($end_ts <= $start_ts) {
            wp_send_json_error(['message' => esc_html__('Invalid schedule range.', 'autopuzzle')]);
        }
    ...
    public function ajax_book_meeting() {
        ...
        if (!empty($exists)) {
            wp_send_json_error(['message' => esc_html__('Selected slot is no longer available.', 'autopuzzle')]);
        }

        $title = sprintf(esc_html__('Meeting - %s', 'autopuzzle'), date_i18n('Y/m/d H:i', strtotime($start)));
        ...
        if (is_wp_error($post_id)) {
            wp_send_json_error(['message' => esc_html__('Server error:', 'autopuzzle') . ' ' . $post_id->get_error_message()]);
        }
        ...
        wp_send_json_success(['message' => esc_html__('Meeting booked successfully.', 'autopuzzle')]);
    }
    ...
    public function ajax_confirm_car_catalog() {
        check_ajax_referer('autopuzzle_confirm_car_catalog_nonce', 'nonce');
        if (!is_user_logged_in()) {
            wp_send_json_error(['message' => esc_html__('Unauthorized access.', 'autopuzzle')], 403);
            return;
        }
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) { $query->the_post();
                ...
                echo '<div class="text-center mt-2"><small class="text-muted">' . esc_html__('Click to select', 'autopuzzle') . '</small></div>';
                echo '</div>';
                echo '</div>'; // Close col
            }
        }
    ...
    public function handle_car_selection_ajax() {
        try {
            // Debug logging
            error_log('AutoPuzzle Debug: handle_car_selection_ajax called');
            error_log('AutoPuzzle Debug: POST data: ' . print_r($_POST, true));
            error_log('AutoPuzzle Debug: User logged in: ' . (is_user_logged_in() ? 'yes' : 'no'));
            error_log('AutoPuzzle Debug: Nonce received: ' . (isset($_POST['nonce']) ? substr($_POST['nonce'], 0, 10) . '...' : 'MISSING'));
            ...
            // Verify nonce with the expected action 'autopuzzle_ajax_nonce'
            $nonce_valid = !empty($nonce) && wp_verify_nonce($nonce, 'autopuzzle_ajax_nonce');
            
            if (!$nonce_valid) {
                error_log('AutoPuzzle Debug: Nonce verification failed');
                error_log('AutoPuzzle Debug: Expected action: autopuzzle_ajax_nonce');
                error_log('AutoPuzzle Debug: Received nonce: ' . ($nonce ? substr($nonce, 0, 20) . '...' : 'EMPTY'));
                error_log('AutoPuzzle Debug: POST keys: ' . implode(', ', array_keys($_POST)));
                wp_send_json_error(['message' => esc_html__('Invalid security token. Please refresh the page and try again.', 'autopuzzle')]);
                return;
            }
            
            error_log('AutoPuzzle Debug: Nonce verified successfully');

            if (!is_user_logged_in()) {
                error_log('AutoPuzzle Debug: User not logged in');
                wp_send_json_error(['message' => esc_html__('Please log in to continue.', 'autopuzzle')]);
                return;
            }
            ...
            if (empty($_POST['product_id'])) {
                error_log('AutoPuzzle Debug: Product ID missing');
                wp_send_json_error(['message' => esc_html__('Product ID is required.', 'autopuzzle')]);
                return;
            }
            ...
            if (!$product) {
                error_log('AutoPuzzle Debug: Product not found: ' . $product_id);
            ...
            if ($total_price_post > 0) {
                ...
                error_log('AutoPuzzle Debug: Using POST total_price: ' . $total_price);
            } else {
                $total_price = $installment_price;
                error_log('AutoPuzzle Debug: Using product installment_price: ' . $total_price);
            }
            ...
            if ($min_down_payment <= 0 && $total_price > 0) {
                ...
                error_log('AutoPuzzle Debug: Recalculated min_down_payment from total_price: ' . $min_down_payment);
                ...
                if ($installment_price > 0 && $total_price != $installment_price) {
                    ...
                    error_log('AutoPuzzle Debug: Adjusted min_down_payment by ratio ' . $ratio . ': ' . $min_down_payment);
                }
            }
            ...
            error_log('AutoPuzzle Debug: Down payment validation - POST: ' . $down_payment_post . ', Min: ' . $min_down_payment . ', Max: ' . $max_down_payment . ', Total Price: ' . $total_price . ', Installment Price: ' . $installment_price);
            ...
            if ($down_payment_post > 0) {
                ...
                if ($down_payment_post >= $min_threshold && $down_payment_post <= $max_down_payment) {
                    ...
                    error_log('AutoPuzzle Debug: ✅ Using POST down_payment (passed flexible validation): ' . $down_payment);
                } elseif ($down_payment_post >= $min_down_payment && $down_payment_post <= $max_down_payment) {
                    $down_payment = $down_payment_post;
                    error_log('AutoPuzzle Debug: ✅ Using POST down_payment (passed strict validation): ' . $down_payment);
                } else {
                    // POST value is outside range - use it anyway if it's at least 10% of total_price
                    if ($down_payment_post >= $min_threshold) {
                        $down_payment = $down_payment_post;
                        error_log('AutoPuzzle Debug: ✅ Using POST down_payment (below min but above 10% threshold): ' . $down_payment);
                    } else {
                        $down_payment = max($min_down_payment, $min_threshold);
                        error_log('AutoPuzzle Debug: ⚠️ POST down_payment (' . $down_payment_post . ') too small, using: ' . $down_payment);
                    }
                }
            } else {
                $down_payment = $min_down_payment;
                error_log('AutoPuzzle Debug: No POST down_payment, using min_down_payment: ' . $down_payment);
            }
            ...
            if ($loan_amount <= 0) {
                error_log('AutoPuzzle Debug: Invalid loan amount calculated: ' . $loan_amount);
                wp_send_json_error(['message' => esc_html__('Invalid loan amount. Please check the down payment.', 'autopuzzle')]);
                return;
            }
            ...
            if (!class_exists('Autopuzzle_Render_Helpers')) {
                error_log('AutoPuzzle Debug: Autopuzzle_Render_Helpers class not found');
                wp_send_json_error(['message' => esc_html__('Server error: Helper class not found.', 'autopuzzle')]);
                return;
            }
            
            // Check if method exists
            if (!method_exists('Autopuzzle_Render_Helpers', 'calculate_installment_amount')) {
                error_log('AutoPuzzle Debug: calculate_installment_amount method not found');
                wp_send_json_error(['message' => esc_html__('Server error: Calculation method not found.', 'autopuzzle')]);
                return;
            }
            
            try {
                $recalculated_installment = Autopuzzle_Render_Helpers::calculate_installment_amount($loan_amount, $term_months);
                
                if ($recalculated_installment <= 0) {
                    error_log('AutoPuzzle Debug: Invalid installment calculated: ' . $recalculated_installment);
                    wp_send_json_error(['message' => esc_html__('Invalid installment amount calculated. Please try again.', 'autopuzzle')]);
                    return;
                }
            } catch (Exception $e) {
                error_log('AutoPuzzle Debug: Exception in calculate_installment_amount: ' . $e->getMessage());
                error_log('AutoPuzzle Debug: Stack trace: ' . $e->getTraceAsString());
                wp_send_json_error(['message' => esc_html__('Server error: Failed to calculate installment.', 'autopuzzle') . ' ' . $e->getMessage()]);
                return;
            } catch (Error $e) {
                error_log('AutoPuzzle Debug: Fatal error in calculate_installment_amount: ' . $e->getMessage());
                error_log('AutoPuzzle Debug: Stack trace: ' . $e->getTraceAsString());
                wp_send_json_error(['message' => esc_html__('Server error: Failed to calculate installment.', 'autopuzzle') . ' ' . $e->getMessage()]);
                return;
            }
            
            error_log('AutoPuzzle Debug: Car replacement calculation: ' . print_r([
            ...
            $meta_to_save = [
                'autopuzzle_selected_car_id'      => $product_id,
                'autopuzzle_inquiry_step'         => 'form_pending',
                ...
                'autopuzzle_inquiry_installment'  => $recalculated_installment, // Use server-calculated value
            ];

            error_log('AutoPuzzle Debug: Saving user meta:');
            ...
            foreach ($meta_to_save as $key => $value) {
                ...
                if (in_array($key, ['autopuzzle_inquiry_down_payment', 'autopuzzle_inquiry_total_price', 'autopuzzle_inquiry_installment', 'autopuzzle_selected_car_id'])) {
                    $value = (int)$value;
                } elseif ($key === 'autopuzzle_inquiry_term_months') {
                    $value = (int)$value;
                }
            ...
            error_log('AutoPuzzle Debug: Car selection successful for user ' . $user_id . ', product ' . $product_id);
            
            // Return updated values so frontend can display them
            wp_send_json_success([
                'message' => esc_html__('Car replaced successfully.', 'autopuzzle'),
                'car_data' => [
                    'total_price' => $total_price,
                    'down_payment' => $down_payment,
                    'term_months' => $term_months,
            ...
            error_log('AutoPuzzle Debug: Exception in handle_car_selection_ajax: ' . $e->getMessage());
            error_log('AutoPuzzle Debug: Exception file: ' . $e->getFile() . ':' . $e->getLine());
            error_log('AutoPuzzle Debug: Stack trace: ' . $e->getTraceAsString());
            wp_send_json_error(['message' => esc_html__('Server error occurred. Please try again.', 'autopuzzle') . ' ' . (defined('WP_DEBUG') && WP_DEBUG ? $e->getMessage() : '')]);
        } catch (Error $e) {
            error_log('AutoPuzzle Debug: Fatal error in handle_car_selection_ajax: ' . $e->getMessage());
            error_log('AutoPuzzle Debug: Fatal error file: ' . $e->getFile() . ':' . $e->getLine());
            error_log('AutoPuzzle Debug: Stack trace: ' . $e->getTraceAsString());
            wp_send_json_error(['message' => esc_html__('Server error occurred. Please try again.', 'autopuzzle') . ' ' . (defined('WP_DEBUG') && WP_DEBUG ? $e->getMessage() : '')]);
        }
    }
    ...
    public function handle_identity_submission() {
        if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'autopuzzle_submit_identity_nonce')) {
        ...
        foreach ($required_fields as $field) {
            if (empty($_POST[$field])) {
                wp_die(sprintf(esc_html__('Error: The field "%s" is required.', 'autopuzzle'), $field));
            }
        }
        ...
        update_user_meta($user_id, 'autopuzzle_temp_inquiry_data', [
        ...
        update_user_meta($user_id, 'autopuzzle_inquiry_step', 'confirm_car_pending');
    ...
    public function handle_confirm_car_step() {
        if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'autopuzzle_confirm_car_step_nonce')) {
            wp_die(esc_html__('Security check failed!', 'autopuzzle'));
        }
        $user_id = 0;
        ...
        if (empty($_POST['confirm_car_agree'])) {
            wp_die(esc_html__('Please confirm your selected car to continue.', 'autopuzzle'));
        }

        // Check if payment is required
        $payment_enabled = Autopuzzle_Options_Helper::is_option_enabled('payment_enabled', false);
        $inquiry_fee = (int)Autopuzzle_Options_Helper::get_option('inquiry_fee', 0);

        if ($payment_enabled && $inquiry_fee > 0) {
            // Payment is required - move to payment step
            update_user_meta($user_id, 'autopuzzle_inquiry_step', 'payment_pending');
            ...
            delete_user_meta($user_id, 'autopuzzle_inquiry_step');
    ...
    /**
     * Callback for the hook `autopuzzle_inquiry_payment_successful`.
    ...
    /**
     * Callback for the hook `autopuzzle_cash_inquiry_payment_successful`.
    ...
    public function finalize_inquiry($user_id, $is_new = false) {
        $temp_data = get_user_meta($user_id, 'autopuzzle_temp_inquiry_data', true);
        if (empty($temp_data)) {
            return false;
        }
        ...
        if (is_wp_error($post_id)) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log('AutoPuzzle Inquiry Error: Failed to insert post. ' . $post_id->get_error_message());
                }
            }
            return false;
        }
        ...
        if ($user_id > 0) {
            $customer_notification = Autopuzzle_Notification_Handler::create_notification(array(
                'user_id' => $user_id,
                'type' => 'inquiry_new',
                'title' => esc_html__('Your installment request has been submitted', 'autopuzzle'),
                'message' => sprintf(esc_html__('Your installment request for %s has been successfully submitted and is being reviewed', 'autopuzzle'), $car_name),
                'link' => home_url('/dashboard/inquiries/installment?inquiry_id=' . $post_id),
                'related_id' => $post_id,
            ));
            if (defined('WP_DEBUG') && WP_DEBUG && !$customer_notification) {
                error_log('AutoPuzzle: Failed to create customer notification for installment inquiry ' . $post_id . ', user_id: ' . $user_id);
            }
        }
        ...
        foreach ($managers as $manager_id) {
            $manager_notification = Autopuzzle_Notification_Handler::create_notification(array(
                ...
                'message' => sprintf(esc_html__('A new installment inquiry from %s for %s has been registered', 'autopuzzle'), $customer_name, $car_name),
                'link' => home_url('/dashboard/inquiries/installment?inquiry_id=' . $post_id),
                'related_id' => $post_id,
            ));
            if (defined('WP_DEBUG') && WP_DEBUG && !$manager_notification) {
                error_log('AutoPuzzle: Failed to create manager notification for installment inquiry ' . $post_id . ', manager_id: ' . $manager_id);
            }
        }
        ...
        $inquiry_fee_amount = (int)Autopuzzle_Options_Helper::get_option('inquiry_fee', 0);
        ...
        $transaction_data = get_user_meta($user_id, 'autopuzzle_last_payment_transaction', true);
        if (!empty($transaction_data)) {
            ...
            // Clean up user meta
            delete_user_meta($user_id, 'autopuzzle_last_payment_transaction');
        }
        ...
        if (!$product) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('AutoPuzzle Error: Product not found for car_id: ' . $car_id);
            }
            return false;
        }
        ...
        if ($total_price <= 0) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('AutoPuzzle Warning: Installment price not set for product ID: ' . $car_id . '. Inquiry may have invalid pricing.');
            }
            // Try to get from user meta as fallback (for backward compatibility with old inquiries)
            $total_price = get_user_meta($user_id, 'autopuzzle_inquiry_total_price', true);
            if ($total_price <= 0) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log('AutoPuzzle Error: Could not determine total price for inquiry. Product ID: ' . $car_id);
                }
                return false;
            }
        }
        ...
        $down_payment = get_user_meta($user_id, 'autopuzzle_inquiry_down_payment', true);
        $term_months = get_user_meta($user_id, 'autopuzzle_inquiry_term_months', true);
        
        // Validate down payment and term months
        if (empty($down_payment) || $down_payment <= 0) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('AutoPuzzle Error: Invalid down payment for inquiry. User ID: ' . $user_id);
            }
            return false;
        }
        ...
        if ($loan_amount <= 0) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('AutoPuzzle Error: Invalid loan amount calculated. Total price: ' . $total_price . ', Down payment: ' . $down_payment);
            }
            return false;
        }
        
        // USE CENTRALIZED HELPER to recalculate installment
        $recalculated_installment = Autopuzzle_Render_Helpers::calculate_installment_amount($loan_amount, (int)$term_months);
        
        // Save the current prices and user selections to post meta
        update_post_meta($post_id, 'autopuzzle_inquiry_down_payment', (int)$down_payment);
        update_post_meta($post_id, 'autopuzzle_inquiry_term_months', (int)$term_months);
        update_post_meta($post_id, 'autopuzzle_inquiry_total_price', (int)$total_price);
        
        // IMPORTANT: Overwrite/Save the final, server-calculated installment amount
        update_post_meta($post_id, 'autopuzzle_inquiry_installment', $recalculated_installment); 
        ...
        $calculator_meta_keys = [
            ...
            'autopuzzle_discount_applied',
            'autopuzzle_last_payment_transaction'
        ];
    ...
    public function execute_finotex_inquiry($national_code) {
        if (!Autopuzzle_Options_Helper::is_option_enabled('finotex_enabled', false) || !defined('AUTOPUZZLE_FINOTEX_API_URL')) {
        ...
        // FIX: Using AUTOPUZZLE_FINOTEX_API_URL constant
        $api_url = sprintf(AUTOPUZZLE_FINOTEX_API_URL, $client_id);
    ...
    public function handle_inquiry_retry() {
        if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'autopuzzle_retry_inquiry_nonce')) {
            wp_die(esc_html__('Security check failed!', 'autopuzzle'));
        }
        if (!is_user_logged_in() || empty($_POST['inquiry_id'])) {
            wp_die(esc_html__('Invalid request.', 'autopuzzle'));
        }
        ...
        if (get_post_field('post_author', $post_id) != $user_id) {
            wp_die(esc_html__('Access denied.', 'autopuzzle'));
        }

        wp_delete_post($post_id, true);
        update_user_meta($user_id, 'autopuzzle_inquiry_step', 'form_pending');
    ...
    private function cleanup_user_meta($user_id, $calculator_meta_keys) {
        ...
        $keys_to_delete = array_merge(
            $calculator_meta_keys,
            ['autopuzzle_inquiry_step', 'autopuzzle_selected_car_id', 'autopuzzle_temp_inquiry_data']
        );
