/*
 * Persian Jalali Datepicker for jQuery UI (Final, Complete & Corrected Version)
 * Written by Meysam Pourmansouri - Final corrections by Gemini for Maneli-Car-Inquiry
 */
(function($) {
        ...
        main: {
            ...
            persianDatepicker: function() {
                ...
                e._adjustInstDate = function(a, b, c) {
                    ...
                    var d = a.drawMonth;
                    if (c == "M") {
                        var g = h.main.getGregorianDate(f, d + b + 1);
                        a.drawMonth = g.gMonth - 1;
                        a.drawYear = g.gYear
                    } else {
                        var g = h.main.getGregorianDate(f+b, d + 1);
						a.drawMonth = g.gMonth - 1;
						a.drawYear = g.gYear;
                    }
                };
                ...
                e._generateHTML = function(S) {
					...
					var v = S.drawMonth;
					var D = S.drawYear;
					var P = h.main.getJalaliDate(new Date(S.selectedYear, S.selectedMonth, S.selectedDay));
					if (J) {
						var l = this._daylightSavingAdjust(new Date(J.getFullYear(), J.getMonth(), J.getDate()));
						l = l < x ? x : l;
						...
						for (var V = 0; V < B[1]; V++) {
							...
							for (var ag = 0; ag < Math.ceil((ae + Z) / 7); ag++) {
								...
								for (var ac = 0; ac < 7; ac++) {
									...
									if( af < Z)
									{
										var prevJMonth = W.jMonth-1;
										var prevJYear = W.jYear;
										if( prevJMonth==0)
										{
											prevJMonth = 12;
											prevJYear--;
										}
										var prevMonthDays = aJ(prevJYear,prevJMonth);
										var ak_d = prevMonthDays - Z + af + 1;
										var gDate = h.main.getGregorianDate( prevJYear, prevJMonth, ak_d);
										aj = new Date( gDate.gYear, gDate.gMonth-1, gDate.gDay);
									}else{
										var gDate = h.main.getGregorianDate( W.jYear, W.jMonth, af - Z + 1);
										aj = new Date( gDate.gYear, gDate.gMonth-1, gDate.gDay);
									}
									var ak = h.main.getJalaliDate(aj);
									var al = ak.jMonth != W.jMonth;
									var am = (N && al) || !al;
									var an = [true, am ? "" : "ui-datepicker-other-month", ak.jDay == P.jDay && ak.jMonth == P.jMonth && ak.jYear == P.jYear ? "ui-datepicker-days-cell-over" : "", al ? "ui-datepicker-unselectable ui-state-disabled" : ""];
									var ao = Q ? Q.apply(S.input ? S.input[0] : null, [aj]) : [true, ""];
									an[1] += ao[1];
									var ap = x && aj < x;
									var aq = J && aj > J;
									ah += "<td class='" + an[1] + ((ac + m + 6) % 7 >= 5 ? " ui-datepicker-week-end" : "") + (an[2] ? " " + an[2] : "") + (al || !an[0] || ap || aq ? " " + an[3] : "") + "' data-handler='selectDay' data-event='click' data-month='" + (ak.jMonth - 1) + "' data-year='" + ak.jYear + "'>" + (al && !y ? "&#xa0;" : (am ? "<a class='ui-state-default" + (ak.jDay == P.jDay && ak.jMonth == P.jMonth && ak.jYear == P.jYear ? " ui-state-active" : "") + (al ? " ui-priority-secondary" : "") + "' href='#'>" + ak.jDay + "</a>" : "<span class='ui-state-default" + (ak.jDay == P.jDay && ak.jMonth == P.jMonth && ak.jYear == P.jYear ? " ui-state-active" : "") + (al ? " ui-priority-secondary" : "") + "'>" + ak.jDay + "</span>")) + "</td>";
									af++
								}
								c += ah + "</tr>"
							}
			...
			onChangeMonthYear: function(b, f, d, c) {
				var gDate = h.main.getGregorianDate(f,b+1,1);
				d.drawMonth = gDate.gMonth-1;
				d.drawYear = gDate.gYear;
				d.selectedYear = f;
				d.selectedMonth = b;
				if (c.onChangeMonthYear) {
					c.onChangeMonthYear(f,b,d);
				}
			},
